...
// Create page object
$rates_edit = new crates_edit();
$Page =& $rates_edit;

// Page init
$rates_edit->Page_Init();

// Page main
$rates_edit->Page_Main();
?>
...
// Create page object
var rates_edit = new ew_Page("rates_edit");

// page properties
rates_edit.PageID = "edit"; // page ID
rates_edit.FormID = "fratesedit"; // form ID
var EW_PAGE_ID = rates_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
rates_edit.ValidateForm = function(fobj) {
...
rates_edit.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
rates_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
rates_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
rates_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $rates->TableCaption() ?><br><br>
<a href="<?php echo $rates->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$rates_edit->ShowMessage();
?>
<form name="fratesedit" id="fratesedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return rates_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="rates">
...
<?php if ($rates->id->Visible) { // id ?>
	<tr<?php echo $rates->id->RowAttributes ?>>
		...
		<td<?php echo $rates->id->CellAttributes() ?>><span id="el_id">
<div<?php echo $rates->id->ViewAttributes() ?>><?php echo $rates->id->EditValue ?></div><input type="hidden" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($rates->id->CurrentValue) ?>">
</span><?php echo $rates->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<?php } else { ?>
<?php $rates->Client_ID->EditAttrs["onchange"] = "ew_UpdateOpt('x_Origin_ID','x_Client_ID',rates_edit.ar_x_Origin_ID);ew_UpdateOpt('x_Destination_ID','x_Client_ID',rates_edit.ar_x_Destination_ID); " . @$rates->Client_ID->EditAttrs["onchange"]; ?>
...
<script type="text/javascript">
<!--
rates_edit.ar_x_Origin_ID = [<?php echo $jswrk ?>];
...
<script type="text/javascript">
<!--
rates_edit.ar_x_Destination_ID = [<?php echo $jswrk ?>];
...
<script language="JavaScript" type="text/javascript">
<!--
ew_UpdateOpts([['x_Origin_ID','x_Client_ID',rates_edit.ar_x_Origin_ID],
['x_Destination_ID','x_Client_ID',rates_edit.ar_x_Destination_ID]]);
...
class crates_edit {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$rates->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$rates->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$rates->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$rates->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($rates->id->CurrentValue == "")
			$this->Page_Terminate("rateslist.php"); // Invalid key, return to list
		switch ($rates->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$rates->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$rates->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		} elseif ($rates->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// id
			$rates->id->EditCustomAttributes = "";
			$rates->id->EditValue = $rates->id->CurrentValue;
			$rates->id->CssStyle = "";
			$rates->id->CssClass = "";
			$rates->id->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$bUpdateRow = $rates->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($rates->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
