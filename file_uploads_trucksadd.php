...
// Create page object
$file_uploads_trucks_add = new cfile_uploads_trucks_add();
$Page =& $file_uploads_trucks_add;

// Page init
$file_uploads_trucks_add->Page_Init();

// Page main
$file_uploads_trucks_add->Page_Main();
?>
...
// Create page object
var file_uploads_trucks_add = new ew_Page("file_uploads_trucks_add");

// page properties
file_uploads_trucks_add.PageID = "add"; // page ID
file_uploads_trucks_add.FormID = "ffile_uploads_trucksadd"; // form ID
var EW_PAGE_ID = file_uploads_trucks_add.PageID; // for backward compatibility

// extend page with ValidateForm function
file_uploads_trucks_add.ValidateForm = function(fobj) {
...
file_uploads_trucks_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
file_uploads_trucks_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
file_uploads_trucks_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
file_uploads_trucks_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $file_uploads_trucks->TableCaption() ?><br><br>
<a href="<?php echo $file_uploads_trucks->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$file_uploads_trucks_add->ShowMessage();
?>
<form name="ffile_uploads_trucksadd" id="ffile_uploads_trucksadd" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return file_uploads_trucks_add.ValidateForm(this);">
<p>
...
<?php
$file_uploads_trucks_add->Page_Terminate();
?>
<?php
...
// Page class
//
class cfile_uploads_trucks_add {
	...
	// Page object name
	var $PageObjName = 'file_uploads_trucks_add';
	...
	//
	function cfile_uploads_trucks_add() {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $file_uploads_trucks->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->GetUploadFiles(); // Get upload files
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$file_uploads_trucks->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $file_uploads_trucks->CurrentAction = "C"; // Copy record
		  } else {
		    $file_uploads_trucks->CurrentAction = "I"; // Display blank record
		...
		switch ($file_uploads_trucks->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$file_uploads_trucks->SendEmail = TRUE; // Send email on add success
		...
		$file_uploads_trucks->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $file_uploads_trucks;
		$file_uploads_trucks->Filename->CurrentValue = NULL; // Clear file related field
	}
	...
	function RenderRow() {
		...
		} elseif ($file_uploads_trucks->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		$bInsertRow = $file_uploads_trucks->Row_Inserting($rsnew);
		if ($bInsertRow) {
			...
			$AddRow = $conn->Execute($file_uploads_trucks->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$file_uploads_trucks->id->setDbValue($conn->Insert_ID());
			$rsnew['id'] = $file_uploads_trucks->id->DbValue;

			// Call Row Inserted event
			$file_uploads_trucks->Row_Inserted($rsnew);
		}
