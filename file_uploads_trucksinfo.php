...
$file_uploads_trucks = NULL;

//
// Table class for file_uploads_trucks
//
class cfile_uploads_trucks {
	var $TableVar = 'file_uploads_trucks';
	...
	//
	function cfile_uploads_trucks() {
		global $Language;

		// id
		$this->id = new cField('file_uploads_trucks', 'file_uploads_trucks', 'x_id', 'id', '`id`', 3, -1, FALSE, '`id`', FALSE);
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] =& $this->id;

		// Trucks
		$this->Trucks = new cField('file_uploads_trucks', 'file_uploads_trucks', 'x_Trucks', 'Trucks', '`Trucks`', 3, -1, FALSE, '`Trucks`', FALSE);
		$this->Trucks->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Trucks'] =& $this->Trucks;

		// Filename
		$this->Filename = new cField('file_uploads_trucks', 'file_uploads_trucks', 'x_Filename', 'Filename', '`Filename`', 200, -1, TRUE, '`Filename`', FALSE);
		$this->Filename->UploadPath = EW_UPLOAD_DEST_PATH;
		$this->fields['Filename'] =& $this->Filename;

		// File_Type_ID
		$this->File_Type_ID = new cField('file_uploads_trucks', 'file_uploads_trucks', 'x_File_Type_ID', 'File_Type_ID', '`File_Type_ID`', 3, -1, FALSE, '`File_Type_ID`', FALSE);
		$this->File_Type_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['File_Type_ID'] =& $this->File_Type_ID;

		// Remarks
		$this->Remarks = new cField('file_uploads_trucks', 'file_uploads_trucks', 'x_Remarks', 'Remarks', '`Remarks`', 201, -1, FALSE, '`Remarks`', FALSE);
		$this->fields['Remarks'] =& $this->Remarks;

		// Created
		$this->Created = new cField('file_uploads_trucks', 'file_uploads_trucks', 'x_Created', 'Created', '`Created`', 135, 6, FALSE, '`Created`', FALSE);
		$this->Created->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateMDY"));
		$this->fields['Created'] =& $this->Created;

		// Modified
		$this->Modified = new cField('file_uploads_trucks', 'file_uploads_trucks', 'x_Modified', 'Modified', '`Modified`', 135, 6, FALSE, '`Modified`', FALSE);
	...
	function HighlightName() {
		return "file_uploads_trucks_Highlight";
	}
	...
	// Master filter
	function SqlMasterFilter_trucks() {
		return "`id`=@id@";
	}

	// Detail filter
	function SqlDetailFilter_trucks() {
		return "`Trucks`=@Trucks@";
	}

	// Table level SQL
	function SqlFrom() { // From
		return "`file_uploads_trucks`";
	}
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `file_uploads_trucks` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		global $conn;
		$SQL = "UPDATE `file_uploads_trucks` SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `file_uploads_trucks` WHERE ";
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "file_uploads_truckslist.php";
		}
	}
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("file_uploads_trucksview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "file_uploads_trucksadd.php";
	...
	// Edit URL
	function EditUrl() {
		return $this->KeyUrl("file_uploads_trucksedit.php", $this->UrlParm());
	}
	...
	// Copy URL
	function CopyUrl() {
		return $this->KeyUrl("file_uploads_trucksadd.php", $this->UrlParm());
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("file_uploads_trucksdelete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=file_uploads_trucks" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function LoadListRowValues(&$rs) {
		...
		$this->Trucks->setDbValue($rs->fields('Trucks'));
   ...
   // Common render codes
		...
		// Trucks
		$this->Trucks->CellCssStyle = ""; $this->Trucks->CellCssClass = "";
		$this->Trucks->CellAttrs = array(); $this->Trucks->ViewAttrs = array(); $this->Trucks->EditAttrs = array();
		...
		// Trucks
		$this->Trucks->ViewValue = $this->Trucks->CurrentValue;
		$this->Trucks->CssStyle = "";
		$this->Trucks->CssClass = "";
		$this->Trucks->ViewCustomAttributes = "";
		...
		// Trucks
		$this->Trucks->HrefValue = "";
		$this->Trucks->TooltipValue = "";
		...
		if (!ew_Empty($this->Filename->Upload->DbValue)) {
			...
			if ($this->Export <> "") $file_uploads_trucks->Filename->HrefValue = ew_ConvertFullUrl($this->Filename->HrefValue);
		} else {
