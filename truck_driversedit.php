...
// Create page object
$truck_drivers_edit = new ctruck_drivers_edit();
$Page =& $truck_drivers_edit;

// Page init
$truck_drivers_edit->Page_Init();

// Page main
$truck_drivers_edit->Page_Main();
?>
...
// Create page object
var truck_drivers_edit = new ew_Page("truck_drivers_edit");

// page properties
truck_drivers_edit.PageID = "edit"; // page ID
truck_drivers_edit.FormID = "ftruck_driversedit"; // form ID
var EW_PAGE_ID = truck_drivers_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
truck_drivers_edit.ValidateForm = function(fobj) {
...
truck_drivers_edit.Form_CustomValidate =  
...
truck_drivers_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
truck_drivers_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $truck_drivers->TableCaption() ?><br><br>
<a href="<?php echo $truck_drivers->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$truck_drivers_edit->ShowMessage();
?>
<form name="ftruck_driversedit" id="ftruck_driversedit" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return truck_drivers_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="truck_drivers">
...
<?php if ($truck_drivers->id->Visible) { // id ?>
	<tr<?php echo $truck_drivers->id->RowAttributes ?>>
		...
		<td<?php echo $truck_drivers->id->CellAttributes() ?>><span id="el_id">
<div<?php echo $truck_drivers->id->ViewAttributes() ?>><?php echo $truck_drivers->id->EditValue ?></div><input type="hidden" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($truck_drivers->id->CurrentValue) ?>">
</span><?php echo $truck_drivers->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<a href="<?php echo $truck_drivers->File_Upload->HrefValue ?>"><?php echo $truck_drivers->File_Upload->EditValue ?></a>
<?php } elseif (!in_array($truck_drivers->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } else { ?>
<?php if (!empty($truck_drivers->File_Upload->Upload->DbValue)) { ?>
<?php echo $truck_drivers->File_Upload->EditValue ?>
<?php } elseif (!in_array($truck_drivers->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } ?>
...
<?php $truck_drivers->File_Upload->EditAttrs["onchange"] = "this.form.a_File_Upload[2].checked=true;" . @$truck_drivers->File_Upload->EditAttrs["onchange"]; ?>
<?php } else { ?>
...
<?php
$truck_drivers_edit->Page_Terminate();
?>
<?php
...
class ctruck_drivers_edit {
	...
	// Page object name
	var $PageObjName = 'truck_drivers_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$truck_drivers->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->GetUploadFiles(); // Get upload files
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$truck_drivers->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$truck_drivers->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$truck_drivers->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($truck_drivers->id->CurrentValue == "")
			$this->Page_Terminate("truck_driverslist.php"); // Invalid key, return to list
		switch ($truck_drivers->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$truck_drivers->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$truck_drivers->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		if ($truck_drivers->RowType == EW_ROWTYPE_VIEW) { // View row
		...
		} elseif ($truck_drivers->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// id
			$truck_drivers->id->EditCustomAttributes = "";
			$truck_drivers->id->EditValue = $truck_drivers->id->CurrentValue;
			$truck_drivers->id->CssStyle = "";
			$truck_drivers->id->CssClass = "";
			$truck_drivers->id->ViewCustomAttributes = "";
			...
			// File_Upload
			if (!ew_Empty($truck_drivers->File_Upload->Upload->DbValue)) {
				$truck_drivers->File_Upload->HrefValue = ew_UploadPathEx(FALSE, $truck_drivers->File_Upload->UploadPath) . ((!empty($truck_drivers->File_Upload->EditValue)) ? $truck_drivers->File_Upload->EditValue : $truck_drivers->File_Upload->CurrentValue);
				if ($truck_drivers->Export <> "") $truck_drivers->File_Upload->HrefValue = ew_ConvertFullUrl($truck_drivers->File_Upload->HrefValue);
			} else {
				$truck_drivers->File_Upload->HrefValue = "";
			}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$truck_drivers->File_Upload->Upload->SaveToSession(); // Save file value to Session
						if ($truck_drivers->File_Upload->Upload->Action == "2" || $truck_drivers->File_Upload->Upload->Action == "3") { // Update/Remove
			$truck_drivers->File_Upload->Upload->DbValue = $rs->fields('File_Upload'); // Get original value
			...
			$bUpdateRow = $truck_drivers->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
			if (!ew_Empty($truck_drivers->File_Upload->Upload->Value)) {
				...
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($truck_drivers->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$truck_drivers->Row_Updated($rsold, $rsnew);
		$rs->Close();
