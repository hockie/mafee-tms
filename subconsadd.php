...
// Create page object
$subcons_add = new csubcons_add();
$Page =& $subcons_add;

// Page init
$subcons_add->Page_Init();

// Page main
$subcons_add->Page_Main();
?>
...
// Create page object
var subcons_add = new ew_Page("subcons_add");

// page properties
subcons_add.PageID = "add"; // page ID
subcons_add.FormID = "fsubconsadd"; // form ID
var EW_PAGE_ID = subcons_add.PageID; // for backward compatibility

// extend page with ValidateForm function
subcons_add.ValidateForm = function(fobj) {
...
subcons_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
subcons_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
subcons_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
subcons_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $subcons->TableCaption() ?><br><br>
<a href="<?php echo $subcons->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$subcons_add->ShowMessage();
?>
<form name="fsubconsadd" id="fsubconsadd" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return subcons_add.ValidateForm(this);">
<p>
...
class csubcons_add {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $subcons->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->GetUploadFiles(); // Get upload files
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$subcons->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $subcons->CurrentAction = "C"; // Copy record
		  } else {
		    $subcons->CurrentAction = "I"; // Display blank record
		...
		switch ($subcons->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$subcons->SendEmail = TRUE; // Send email on add success
		...
		$subcons->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $subcons;
		$subcons->File_Upload->CurrentValue = NULL; // Clear file related field
	}
	...
	function RenderRow() {
		...
		} elseif ($subcons->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		$bInsertRow = $subcons->Row_Inserting($rsnew);
		if ($bInsertRow) {
			...
			$AddRow = $conn->Execute($subcons->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$subcons->id->setDbValue($conn->Insert_ID());
			$rsnew['id'] = $subcons->id->DbValue;

			// Call Row Inserted event
			$subcons->Row_Inserted($rsnew);
		}
