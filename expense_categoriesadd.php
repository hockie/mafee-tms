...
// Create page object
$expense_categories_add = new cexpense_categories_add();
$Page =& $expense_categories_add;

// Page init
$expense_categories_add->Page_Init();

// Page main
$expense_categories_add->Page_Main();
?>
...
// Create page object
var expense_categories_add = new ew_Page("expense_categories_add");

// page properties
expense_categories_add.PageID = "add"; // page ID
expense_categories_add.FormID = "fexpense_categoriesadd"; // form ID
var EW_PAGE_ID = expense_categories_add.PageID; // for backward compatibility

// extend page with ValidateForm function
expense_categories_add.ValidateForm = function(fobj) {
...
expense_categories_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
expense_categories_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
expense_categories_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
expense_categories_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $expense_categories->TableCaption() ?><br><br>
<a href="<?php echo $expense_categories->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$expense_categories_add->ShowMessage();
?>
<form name="fexpense_categoriesadd" id="fexpense_categoriesadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return expense_categories_add.ValidateForm(this);">
<p>
...
<?php
$expense_categories_add->Page_Terminate();
?>
<?php
...
class cexpense_categories_add {
	...
	// Page object name
	var $PageObjName = 'expense_categories_add';
	...
	//
	function cexpense_categories_add() {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $expense_categories->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$expense_categories->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $expense_categories->CurrentAction = "C"; // Copy record
		  } else {
		    $expense_categories->CurrentAction = "I"; // Display blank record
		...
		switch ($expense_categories->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$expense_categories->SendEmail = TRUE; // Send email on add success
		...
		$expense_categories->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $expense_categories;
		$expense_categories->cost->CurrentValue = 0.00;
	}
	...
	function LoadFormValues() {
		...
		$expense_categories->created->setFormValue($objForm->GetValue("x_created"));
		$expense_categories->created->CurrentValue = ew_UnFormatDateTime($expense_categories->created->CurrentValue, 6);
	...
	function RestoreFormValues() {
		...
		$expense_categories->created->CurrentValue = $expense_categories->created->FormValue;
		$expense_categories->created->CurrentValue = ew_UnFormatDateTime($expense_categories->created->CurrentValue, 6);
	...
	function RenderRow() {
		...
		} elseif ($expense_categories->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		// created
		$expense_categories->created->SetDbValueDef($rsnew, ew_CurrentDateTime(), NULL);
		$rsnew['created'] =& $expense_categories->created->DbValue;
		...
		$bInsertRow = $expense_categories->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($expense_categories->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$expense_categories->id->setDbValue($conn->Insert_ID());
			$rsnew['id'] = $expense_categories->id->DbValue;

			// Call Row Inserted event
			$expense_categories->Row_Inserted($rsnew);
		}
		return $AddRow;
	}
