...
// Create page object
$vendor_bill_items_edit = new cvendor_bill_items_edit();
$Page =& $vendor_bill_items_edit;

// Page init
$vendor_bill_items_edit->Page_Init();

// Page main
$vendor_bill_items_edit->Page_Main();
?>
...
// Create page object
var vendor_bill_items_edit = new ew_Page("vendor_bill_items_edit");

// page properties
vendor_bill_items_edit.PageID = "edit"; // page ID
vendor_bill_items_edit.FormID = "fvendor_bill_itemsedit"; // form ID
var EW_PAGE_ID = vendor_bill_items_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
vendor_bill_items_edit.ValidateForm = function(fobj) {
...
vendor_bill_items_edit.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
vendor_bill_items_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
vendor_bill_items_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
vendor_bill_items_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $vendor_bill_items->TableCaption() ?><br><br>
<a href="<?php echo $vendor_bill_items->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$vendor_bill_items_edit->ShowMessage();
?>
<form name="fvendor_bill_itemsedit" id="fvendor_bill_itemsedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return vendor_bill_items_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="vendor_bill_items">
...
<?php if ($vendor_bill_items->id->Visible) { // id ?>
	<tr<?php echo $vendor_bill_items->id->RowAttributes ?>>
		...
		<td<?php echo $vendor_bill_items->id->CellAttributes() ?>><span id="el_id">
<div<?php echo $vendor_bill_items->id->ViewAttributes() ?>><?php echo $vendor_bill_items->id->EditValue ?></div><input type="hidden" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($vendor_bill_items->id->CurrentValue) ?>">
</span><?php echo $vendor_bill_items->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<?php } else { ?>
<?php $vendor_bill_items->vendor_id->EditAttrs["onchange"] = "ew_UpdateOpt('x_booking_id','x_vendor_id',vendor_bill_items_edit.ar_x_booking_id); " . @$vendor_bill_items->vendor_id->EditAttrs["onchange"]; ?>
...
<script type="text/javascript">
<!--
vendor_bill_items_edit.ar_x_booking_id = [<?php echo $jswrk ?>];
...
<script language="JavaScript" type="text/javascript">
<!--
ew_UpdateOpts([['x_booking_id','x_vendor_id',vendor_bill_items_edit.ar_x_booking_id]]);
...
<?php
$vendor_bill_items_edit->Page_Terminate();
?>
<?php
...
class cvendor_bill_items_edit {
	...
	// Page object name
	var $PageObjName = 'vendor_bill_items_edit';
	...
	//
	function cvendor_bill_items_edit() {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$vendor_bill_items->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$vendor_bill_items->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$vendor_bill_items->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$vendor_bill_items->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($vendor_bill_items->id->CurrentValue == "")
			$this->Page_Terminate("vendor_bill_itemslist.php"); // Invalid key, return to list
		switch ($vendor_bill_items->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$vendor_bill_items->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$vendor_bill_items->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		} elseif ($vendor_bill_items->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// id
			$vendor_bill_items->id->EditCustomAttributes = "";
			$vendor_bill_items->id->EditValue = $vendor_bill_items->id->CurrentValue;
			$vendor_bill_items->id->CssStyle = "";
			$vendor_bill_items->id->CssClass = "";
			$vendor_bill_items->id->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$bUpdateRow = $vendor_bill_items->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($vendor_bill_items->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$vendor_bill_items->Row_Updated($rsold, $rsnew);
		$rs->Close();
		return $EditRow;
	}
