...
// Create page object
$customers_add = new ccustomers_add();
$Page =& $customers_add;
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $customers->TableCaption() ?><br><br>
...
?>
<form name="fcustomersadd" id="fcustomersadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return customers_add.ValidateForm(this);">
<p>
...
class ccustomers_add {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $customers->CurrentAction = $_POST["a_add"]; // Get form action
	...
	function RenderRow() {
		...
		} elseif ($customers->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		$bInsertRow = $customers->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($customers->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$customers->id->setDbValue($conn->Insert_ID());
			$rsnew['id'] = $customers->id->DbValue;

			// Call Row Inserted event
			$customers->Row_Inserted($rsnew);
		}
		return $AddRow;
	}
