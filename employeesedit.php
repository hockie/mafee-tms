...
// Create page object
$employees_edit = new cemployees_edit();
$Page =& $employees_edit;
...
// Create page object
var employees_edit = new ew_Page("employees_edit");

// page properties
employees_edit.PageID = "edit"; // page ID
employees_edit.FormID = "femployeesedit"; // form ID
var EW_PAGE_ID = employees_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
employees_edit.ValidateForm = function(fobj) {
...
employees_edit.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
employees_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
employees_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
employees_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $employees->TableCaption() ?><br><br>
...
?>
<form name="femployeesedit" id="femployeesedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return employees_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="employees">
...
<?php if ($employees->id->Visible) { // id ?>
	<tr<?php echo $employees->id->RowAttributes ?>>
		...
		<td<?php echo $employees->id->CellAttributes() ?>><span id="el_id">
<div<?php echo $employees->id->ViewAttributes() ?>><?php echo $employees->id->EditValue ?></div><input type="hidden" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($employees->id->CurrentValue) ?>">
</span><?php echo $employees->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
class cemployees_edit {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$employees->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$employees->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$employees->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$employees->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($employees->id->CurrentValue == "")
			$this->Page_Terminate("employeeslist.php"); // Invalid key, return to list
		switch ($employees->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$employees->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$employees->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		} elseif ($employees->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// id
			$employees->id->EditCustomAttributes = "";
			$employees->id->EditValue = $employees->id->CurrentValue;
			$employees->id->CssStyle = "";
			$employees->id->CssClass = "";
			$employees->id->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// SubconID
			$employees->SubconID->SetDbValueDef($rsnew, $employees->SubconID->CurrentValue, 0, FALSE);
			...
			// EmpRateId
			$employees->EmpRateId->SetDbValueDef($rsnew, $employees->EmpRateId->CurrentValue, 0, FALSE);
			...
			// EmpStatusId
			$employees->EmpStatusId->SetDbValueDef($rsnew, $employees->EmpStatusId->CurrentValue, 0, FALSE);

			// Remarks
			$employees->Remarks->SetDbValueDef($rsnew, $employees->Remarks->CurrentValue, "", FALSE);

			// Password
			$employees->Password->SetDbValueDef($rsnew, $employees->Password->CurrentValue, "", FALSE);

			// Call Row Updating event
			$bUpdateRow = $employees->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($employees->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$employees->Row_Updated($rsold, $rsnew);
		$rs->Close();
		return $EditRow;
	}
