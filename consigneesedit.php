...
// Create page object
$consignees_edit = new cconsignees_edit();
$Page =& $consignees_edit;

// Page init
$consignees_edit->Page_Init();

// Page main
$consignees_edit->Page_Main();
?>
...
// Create page object
var consignees_edit = new ew_Page("consignees_edit");

// page properties
consignees_edit.PageID = "edit"; // page ID
consignees_edit.FormID = "fconsigneesedit"; // form ID
var EW_PAGE_ID = consignees_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
consignees_edit.ValidateForm = function(fobj) {
...
consignees_edit.Form_CustomValidate =  
...
consignees_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
consignees_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $consignees->TableCaption() ?><br><br>
<a href="<?php echo $consignees->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$consignees_edit->ShowMessage();
?>
<form name="fconsigneesedit" id="fconsigneesedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return consignees_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="consignees">
...
<?php if ($consignees->id->Visible) { // id ?>
	<tr<?php echo $consignees->id->RowAttributes ?>>
		...
		<td<?php echo $consignees->id->CellAttributes() ?>><span id="el_id">
<div<?php echo $consignees->id->ViewAttributes() ?>><?php echo $consignees->id->EditValue ?></div><input type="hidden" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($consignees->id->CurrentValue) ?>">
</span><?php echo $consignees->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<?php
$consignees_edit->Page_Terminate();
?>
<?php
...
class cconsignees_edit {
	...
	// Page object name
	var $PageObjName = 'consignees_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$consignees->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$consignees->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$consignees->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$consignees->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($consignees->id->CurrentValue == "")
			$this->Page_Terminate("consigneeslist.php"); // Invalid key, return to list
		switch ($consignees->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$consignees->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$consignees->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		} elseif ($consignees->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// id
			$consignees->id->EditCustomAttributes = "";
			$consignees->id->EditValue = $consignees->id->CurrentValue;
			$consignees->id->CssStyle = "";
			$consignees->id->CssClass = "";
			$consignees->id->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$bUpdateRow = $consignees->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($consignees->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$consignees->Row_Updated($rsold, $rsnew);
		$rs->Close();
		return $EditRow;
	}
