...
// Create page object
$customer_invoices_delete = new ccustomer_invoices_delete();
$Page =& $customer_invoices_delete;

// Page init
$customer_invoices_delete->Page_Init();

// Page main
$customer_invoices_delete->Page_Main();
?>
...
// Create page object
var customer_invoices_delete = new ew_Page("customer_invoices_delete");

// page properties
customer_invoices_delete.PageID = "delete"; // page ID
customer_invoices_delete.FormID = "fcustomer_invoicesdelete"; // form ID
var EW_PAGE_ID = customer_invoices_delete.PageID; // for backward compatibility

// extend page with Form_CustomValidate function
customer_invoices_delete.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
customer_invoices_delete.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
customer_invoices_delete.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
customer_invoices_delete.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
if ($rs = $customer_invoices_delete->LoadRecordset())
	$customer_invoices_deletelTotalRecs = $rs->RecordCount(); // Get record count
if ($customer_invoices_deletelTotalRecs <= 0) { // No record found, exit
	if ($rs)
		$rs->Close();
	$customer_invoices_delete->Page_Terminate("customer_invoiceslist.php"); // Return to list
}
?>
<p><span class="phpmaker"><?php echo $Language->Phrase("Delete") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $customer_invoices->TableCaption() ?><br><br>
<a href="<?php echo $customer_invoices->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$customer_invoices_delete->ShowMessage();
?>
<form action="<?php echo ew_CurrentPage() ?>" method="post">
<p>
<input type="hidden" name="t" id="t" value="customer_invoices">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($customer_invoices_delete->arRecKeys as $key) { ?>
...
<?php echo $customer_invoices->TableCustomInnerHtml ?>
	...
	<tr class="ewTableHeader">
		<td valign="top"><?php echo $customer_invoices->id->FldCaption() ?></td>
		<td valign="top"><?php echo $customer_invoices->Payment_ID->FldCaption() ?></td>
		<td valign="top"><?php echo $customer_invoices->Invoice_ID->FldCaption() ?></td>
		<td valign="top"><?php echo $customer_invoices->Invoice_Bill_Date->FldCaption() ?></td>
		<td valign="top"><?php echo $customer_invoices->Due_Date->FldCaption() ?></td>
		<td valign="top"><?php echo $customer_invoices->Total_Amount_Due->FldCaption() ?></td>
		<td valign="top"><?php echo $customer_invoices->Payment_Status_ID->FldCaption() ?></td>
		<td valign="top"><?php echo $customer_invoices->Status_ID->FldCaption() ?></td>
		<td valign="top"><?php echo $customer_invoices->User_ID->FldCaption() ?></td>
		<td valign="top"><?php echo $customer_invoices->Remarks->FldCaption() ?></td>
	</tr>
	</thead>
	<tbody>
<?php
$customer_invoices_delete->lRecCnt = 0;
$i = 0;
while (!$rs->EOF) {
	$customer_invoices_delete->lRecCnt++;

	// Set row properties
	$customer_invoices->CssClass = "";
	$customer_invoices->CssStyle = "";
	$customer_invoices->RowAttrs = array();
	$customer_invoices->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$customer_invoices_delete->LoadRowValues($rs);

	// Render row
	$customer_invoices_delete->RenderRow();
?>
	<tr<?php echo $customer_invoices->RowAttributes() ?>>
		<td<?php echo $customer_invoices->id->CellAttributes() ?>>
<div<?php echo $customer_invoices->id->ViewAttributes() ?>><?php echo $customer_invoices->id->ListViewValue() ?></div></td>
...
<div<?php echo $customer_invoices->Invoice_Bill_Date->ViewAttributes() ?>><?php echo $customer_invoices->Invoice_Bill_Date->ListViewValue() ?></div></td>
		<td<?php echo $customer_invoices->Due_Date->CellAttributes() ?>>
<div<?php echo $customer_invoices->Due_Date->ViewAttributes() ?>><?php echo $customer_invoices->Due_Date->ListViewValue() ?></div></td>
		<td<?php echo $customer_invoices->Total_Amount_Due->CellAttributes() ?>>
<div<?php echo $customer_invoices->Total_Amount_Due->ViewAttributes() ?>><?php echo $customer_invoices->Total_Amount_Due->ListViewValue() ?></div></td>
		<td<?php echo $customer_invoices->Payment_Status_ID->CellAttributes() ?>>
<div<?php echo $customer_invoices->Payment_Status_ID->ViewAttributes() ?>><?php echo $customer_invoices->Payment_Status_ID->ListViewValue() ?></div></td>
		<td<?php echo $customer_invoices->Status_ID->CellAttributes() ?>>
<div<?php echo $customer_invoices->Status_ID->ViewAttributes() ?>><?php echo $customer_invoices->Status_ID->ListViewValue() ?></div></td>
		<td<?php echo $customer_invoices->User_ID->CellAttributes() ?>>
<div<?php echo $customer_invoices->User_ID->ViewAttributes() ?>><?php echo $customer_invoices->User_ID->ListViewValue() ?></div></td>
		<td<?php echo $customer_invoices->Remarks->CellAttributes() ?>>
<div<?php echo $customer_invoices->Remarks->ViewAttributes() ?>><?php echo $customer_invoices->Remarks->ListViewValue() ?></div></td>
	</tr>
<?php
	$rs->MoveNext();
}
...
<?php
$customer_invoices_delete->Page_Terminate();
?>
<?php
...
// Page class
//
class ccustomer_invoices_delete {
	...
	// Page object name
	var $PageObjName = 'customer_invoices_delete';
	...
	//
	function ccustomer_invoices_delete() {
	...
	function Page_Main() {
		...
		if (@$_GET["id"] <> "") {
			...
			if (!is_numeric($customer_invoices->id->QueryStringValue))
				$this->Page_Terminate("customer_invoiceslist.php"); // Prevent SQL injection, exit
			$sKey .= $customer_invoices->id->QueryStringValue;
		} else {
			$bSingleDelete = FALSE;
		}
		if ($bSingleDelete) {
		...
		if ($nKeySelected <= 0)
			$this->Page_Terminate("customer_invoiceslist.php"); // No key specified, return to list
		...
		foreach ($this->arRecKeys as $sKey) {
			...
			if (!is_numeric($sKeyFld))
				$this->Page_Terminate("customer_invoiceslist.php"); // Prevent SQL injection, return to list
		...
		// SQL constructor in customer_invoices class, customer_invoicesinfo.php

		$customer_invoices->CurrentFilter = $sFilter;

		// Get action
		if (@$_POST["a_delete"] <> "") {
			$customer_invoices->CurrentAction = $_POST["a_delete"];
		} else {
			$customer_invoices->CurrentAction = "I"; // Display record
		}
		switch ($customer_invoices->CurrentAction) {
			case "D": // Delete
				$customer_invoices->SendEmail = TRUE; // Send email on delete success
	...
	function DeleteRows() {
		...
		$DeleteRows = TRUE;
		$sWrkFilter = $customer_invoices->CurrentFilter;

		// Set up filter (SQL WHERE clause) and get return SQL
		// SQL constructor in customer_invoices class, customer_invoicesinfo.php

		$customer_invoices->CurrentFilter = $sWrkFilter;
		...
		if ($DeleteRows) {
			foreach ($rsold as $row) {
				$DeleteRows = $customer_invoices->Row_Deleting($row);
				if (!$DeleteRows) break;
			}
		}
		if ($DeleteRows) {
			...
			foreach ($rsold as $row) {
				...
				$sThisKey .= $row['id'];
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$DeleteRows = $conn->Execute($customer_invoices->DeleteSQL($row)); // Delete
				$conn->raiseErrorFn = '';
				if ($DeleteRows === FALSE)
					break;
				if ($sKey <> "") $sKey .= ", ";
		...
		if ($DeleteRows) {
			foreach ($rsold as $row) {
				$customer_invoices->Row_Deleted($row);
			}	
		}
		return $DeleteRows;
	}
