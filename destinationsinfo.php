...
class cdestinations {
	...
	//
	function cdestinations() {
		global $Language;

		// id
		$this->id = new cField('destinations', 'destinations', 'x_id', 'id', '`id`', 3, -1, FALSE, '`id`', FALSE);
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] =& $this->id;

		// Destination
		$this->Destination = new cField('destinations', 'destinations', 'x_Destination', 'Destination', '`Destination`', 200, -1, FALSE, '`Destination`', FALSE);
		$this->fields['Destination'] =& $this->Destination;

		// Client
		$this->Client = new cField('destinations', 'destinations', 'x_Client', 'Client', '`Client`', 3, -1, FALSE, '`Client`', FALSE);
		$this->Client->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `destinations` ($names) VALUES ($values)";
	}
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("destinationsview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "destinationsadd.php";
	...
	// Edit URL
	function EditUrl() {
		return $this->KeyUrl("destinationsedit.php", $this->UrlParm());
	}
	...
	// Copy URL
	function CopyUrl() {
		return $this->KeyUrl("destinationsadd.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=destinations" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
   ...
   // Common render codes
		...
		// Client
		$this->Client->CellCssStyle = ""; $this->Client->CellCssClass = "";
		$this->Client->CellAttrs = array(); $this->Client->ViewAttrs = array(); $this->Client->EditAttrs = array();
		...
		// Client
		if (strval($this->Client->CurrentValue) <> "") {
			$sFilterWrk = "`id` = " . ew_AdjustSql($this->Client->CurrentValue) . "";
		...
		$sSqlWrk .= " ORDER BY `Client_Name` Asc";
			...
			if ($rswrk && !$rswrk->EOF) { // Lookup values found
				$this->Client->ViewValue = $rswrk->fields('Client_Name');
				$rswrk->Close();
			} else {
				$this->Client->ViewValue = $this->Client->CurrentValue;
			}
		} else {
			$this->Client->ViewValue = NULL;
		}
		$this->Client->CssStyle = "";
		$this->Client->CssClass = "";
