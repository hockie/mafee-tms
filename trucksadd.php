...
// Create page object
$trucks_add = new ctrucks_add();
$Page =& $trucks_add;

// Page init
$trucks_add->Page_Init();

// Page main
$trucks_add->Page_Main();
?>
...
// Create page object
var trucks_add = new ew_Page("trucks_add");

// page properties
trucks_add.PageID = "add"; // page ID
trucks_add.FormID = "ftrucksadd"; // form ID
var EW_PAGE_ID = trucks_add.PageID; // for backward compatibility

// extend page with ValidateForm function
trucks_add.ValidateForm = function(fobj) {
...
trucks_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
trucks_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
trucks_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
trucks_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $trucks->TableCaption() ?><br><br>
<a href="<?php echo $trucks->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$trucks_add->ShowMessage();
?>
<form name="ftrucksadd" id="ftrucksadd" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return trucks_add.ValidateForm(this);">
<p>
...
class ctrucks_add {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $trucks->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->GetUploadFiles(); // Get upload files
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$trucks->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $trucks->CurrentAction = "C"; // Copy record
		  } else {
		    $trucks->CurrentAction = "I"; // Display blank record
		...
		switch ($trucks->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$trucks->SendEmail = TRUE; // Send email on add success
		...
		$trucks->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $trucks;
		$trucks->File_Upload->CurrentValue = NULL; // Clear file related field
	}
	...
	function RenderRow() {
		...
		} elseif ($trucks->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		$bInsertRow = $trucks->Row_Inserting($rsnew);
		if ($bInsertRow) {
			...
			$AddRow = $conn->Execute($trucks->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$trucks->id->setDbValue($conn->Insert_ID());
			$rsnew['id'] = $trucks->id->DbValue;

			// Call Row Inserted event
			$trucks->Row_Inserted($rsnew);
		}
