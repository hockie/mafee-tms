...
if ($rs = $audittrail_delete->LoadRecordset())
	$audittrail_deletelTotalRecs = $rs->RecordCount(); // Get record count
if ($audittrail_deletelTotalRecs <= 0) { // No record found, exit
	if ($rs)
		$rs->Close();
	$audittrail_delete->Page_Terminate("audittraillist.php"); // Return to list
}
?>
<p><span class="phpmaker"><?php echo $Language->Phrase("Delete") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $audittrail->TableCaption() ?><br><br>
...
<?php foreach ($audittrail_delete->arRecKeys as $key) { ?>
...
<?php echo $audittrail->TableCustomInnerHtml ?>
	...
	<tr class="ewTableHeader">
		...
		<td valign="top"><?php echo $audittrail->table->FldCaption() ?></td>
		<td valign="top"><?php echo $audittrail->zfield->FldCaption() ?></td>
	</tr>
	</thead>
	<tbody>
<?php
$audittrail_delete->lRecCnt = 0;
$i = 0;
while (!$rs->EOF) {
	$audittrail_delete->lRecCnt++;

	// Set row properties
	$audittrail->CssClass = "";
	$audittrail->CssStyle = "";
	$audittrail->RowAttrs = array();
	$audittrail->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$audittrail_delete->LoadRowValues($rs);
...
<div<?php echo $audittrail->table->ViewAttributes() ?>><?php echo $audittrail->table->ListViewValue() ?></div></td>
...
class caudittrail_delete {
	...
	function Page_Main() {
		...
		if (@$_GET["id"] <> "") {
			...
			if (!is_numeric($audittrail->id->QueryStringValue))
				$this->Page_Terminate("audittraillist.php"); // Prevent SQL injection, exit
			$sKey .= $audittrail->id->QueryStringValue;
		} else {
			$bSingleDelete = FALSE;
		}
		if ($bSingleDelete) {
		...
		if ($nKeySelected <= 0)
			$this->Page_Terminate("audittraillist.php"); // No key specified, return to list
	...
	function DeleteRows() {
		...
		$DeleteRows = TRUE;
		$sWrkFilter = $audittrail->CurrentFilter;

		// Set up filter (SQL WHERE clause) and get return SQL
		// SQL constructor in audittrail class, audittrailinfo.php

		$audittrail->CurrentFilter = $sWrkFilter;
		$sSql = $audittrail->SQL();
