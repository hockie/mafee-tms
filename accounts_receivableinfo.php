...
//
// Table class for accounts_receivable
//
class caccounts_receivable {
	var $TableVar = 'accounts_receivable';
	...
	//
	function caccounts_receivable() {
		global $Language;

		// Booking_Number
		$this->Booking_Number = new cField('accounts_receivable', 'accounts_receivable', 'x_Booking_Number', 'Booking_Number', '`Booking_Number`', 200, -1, FALSE, '`Booking_Number`', FALSE);
		$this->fields['Booking_Number'] =& $this->Booking_Number;

		// Date
		$this->Date = new cField('accounts_receivable', 'accounts_receivable', 'x_Date', 'Date', '`Date`', 135, 6, FALSE, '`Date`', FALSE);
		$this->Date->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateMDY"));
		$this->fields['Date'] =& $this->Date;

		// Client_ID
		$this->Client_ID = new cField('accounts_receivable', 'accounts_receivable', 'x_Client_ID', 'Client_ID', '`Client_ID`', 3, -1, FALSE, '`Client_ID`', FALSE);
		$this->Client_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Client_ID'] =& $this->Client_ID;

		// Origin_ID
		$this->Origin_ID = new cField('accounts_receivable', 'accounts_receivable', 'x_Origin_ID', 'Origin_ID', '`Origin_ID`', 3, -1, FALSE, '`Origin_ID`', FALSE);
		$this->Origin_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Origin_ID'] =& $this->Origin_ID;

		// Destination_ID
		$this->Destination_ID = new cField('accounts_receivable', 'accounts_receivable', 'x_Destination_ID', 'Destination_ID', '`Destination_ID`', 3, -1, FALSE, '`Destination_ID`', FALSE);
		$this->Destination_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Destination_ID'] =& $this->Destination_ID;

		// Customer_ID
		$this->Customer_ID = new cField('accounts_receivable', 'accounts_receivable', 'x_Customer_ID', 'Customer_ID', '`Customer_ID`', 3, -1, FALSE, '`Customer_ID`', FALSE);
		$this->Customer_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Customer_ID'] =& $this->Customer_ID;

		// Subcon_ID
		$this->Subcon_ID = new cField('accounts_receivable', 'accounts_receivable', 'x_Subcon_ID', 'Subcon_ID', '`Subcon_ID`', 3, -1, FALSE, '`Subcon_ID`', FALSE);
		$this->Subcon_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Subcon_ID'] =& $this->Subcon_ID;

		// Truck_ID
		$this->Truck_ID = new cField('accounts_receivable', 'accounts_receivable', 'x_Truck_ID', 'Truck_ID', '`Truck_ID`', 3, -1, FALSE, '`Truck_ID`', FALSE);
		$this->Truck_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Truck_ID'] =& $this->Truck_ID;

		// ETA
		$this->ETA = new cField('accounts_receivable', 'accounts_receivable', 'x_ETA', 'ETA', '`ETA`', 135, 6, FALSE, '`ETA`', FALSE);
		$this->ETA->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateMDY"));
		$this->fields['ETA'] =& $this->ETA;

		// ETD
		$this->ETD = new cField('accounts_receivable', 'accounts_receivable', 'x_ETD', 'ETD', '`ETD`', 135, 6, FALSE, '`ETD`', FALSE);
		$this->ETD->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateMDY"));
		$this->fields['ETD'] =& $this->ETD;

		// Status_ID
		$this->Status_ID = new cField('accounts_receivable', 'accounts_receivable', 'x_Status_ID', 'Status_ID', '`Status_ID`', 3, -1, FALSE, '`Status_ID`', FALSE);
		$this->Status_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Status_ID'] =& $this->Status_ID;

		// Vat
		$this->Vat = new cField('accounts_receivable', 'accounts_receivable', 'x_Vat', 'Vat', '`Vat`', 4, -1, FALSE, '`Vat`', FALSE);
		$this->Vat->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['Vat'] =& $this->Vat;

		// Total_Sales
		$this->Total_Sales = new cField('accounts_receivable', 'accounts_receivable', 'x_Total_Sales', 'Total_Sales', '`Total_Sales`', 4, -1, FALSE, '`Total_Sales`', FALSE);
		$this->Total_Sales->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['Total_Sales'] =& $this->Total_Sales;

		// Wtax
		$this->Wtax = new cField('accounts_receivable', 'accounts_receivable', 'x_Wtax', 'Wtax', '`Wtax`', 4, -1, FALSE, '`Wtax`', FALSE);
		$this->Wtax->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['Wtax'] =& $this->Wtax;

		// Total_Amount_Due
		$this->Total_Amount_Due = new cField('accounts_receivable', 'accounts_receivable', 'x_Total_Amount_Due', 'Total_Amount_Due', '`Total_Amount_Due`', 4, -1, FALSE, '`Total_Amount_Due`', FALSE);
		$this->Total_Amount_Due->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['Total_Amount_Due'] =& $this->Total_Amount_Due;

		// id
		$this->id = new cField('accounts_receivable', 'accounts_receivable', 'x_id', 'id', '`id`', 3, -1, FALSE, '`id`', FALSE);
	...
	function HighlightName() {
		return "accounts_receivable_Highlight";
	}
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `accounts_receivable` ($names) VALUES ($values)";
	}
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `accounts_receivable` WHERE ";
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "accounts_receivablelist.php";
		}
	}
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("accounts_receivableview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "accounts_receivableadd.php";
	...
	// Edit URL
	function EditUrl() {
		return $this->KeyUrl("accounts_receivableedit.php", $this->UrlParm());
	}
	...
	// Copy URL
	function CopyUrl() {
		return $this->KeyUrl("accounts_receivableadd.php", $this->UrlParm());
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("accounts_receivabledelete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=accounts_receivable" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
   ...
   // Common render codes
		...
		// Booking_Number
		if (!ew_Empty($this->Booking_Number->CurrentValue)) {
			$this->Booking_Number->HrefValue = $this->Booking_Number->CurrentValue;
			if ($this->Export <> "") $accounts_receivable->Booking_Number->HrefValue = ew_ConvertFullUrl($this->Booking_Number->HrefValue);
		} else {
		...
		// Client_ID
		if (!ew_Empty($this->Client_ID->CurrentValue)) {
			$this->Client_ID->HrefValue = $this->Client_ID->CurrentValue;
			if ($this->Export <> "") $accounts_receivable->Client_ID->HrefValue = ew_ConvertFullUrl($this->Client_ID->HrefValue);
		} else {
		...
		if (!ew_Empty($this->id->CurrentValue)) {
			$this->id->HrefValue = $this->id->CurrentValue;
			if ($this->Export <> "") $accounts_receivable->id->HrefValue = ew_ConvertFullUrl($this->id->HrefValue);
		} else {
			$this->id->HrefValue = "";
		}
