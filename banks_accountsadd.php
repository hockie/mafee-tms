...
// Create page object
$banks_accounts_add = new cbanks_accounts_add();
$Page =& $banks_accounts_add;

// Page init
$banks_accounts_add->Page_Init();

// Page main
$banks_accounts_add->Page_Main();
?>
...
// Create page object
var banks_accounts_add = new ew_Page("banks_accounts_add");

// page properties
banks_accounts_add.PageID = "add"; // page ID
banks_accounts_add.FormID = "fbanks_accountsadd"; // form ID
var EW_PAGE_ID = banks_accounts_add.PageID; // for backward compatibility

// extend page with ValidateForm function
banks_accounts_add.ValidateForm = function(fobj) {
...
banks_accounts_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
banks_accounts_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
banks_accounts_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
banks_accounts_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $banks_accounts->TableCaption() ?><br><br>
<a href="<?php echo $banks_accounts->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$banks_accounts_add->ShowMessage();
?>
<form name="fbanks_accountsadd" id="fbanks_accountsadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return banks_accounts_add.ValidateForm(this);">
<p>
...
<?php
$banks_accounts_add->Page_Terminate();
?>
<?php
...
class cbanks_accounts_add {
	...
	// Page object name
	var $PageObjName = 'banks_accounts_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $banks_accounts->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$banks_accounts->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $banks_accounts->CurrentAction = "C"; // Copy record
		  } else {
		    $banks_accounts->CurrentAction = "I"; // Display blank record
		...
		switch ($banks_accounts->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$banks_accounts->SendEmail = TRUE; // Send email on add success
		...
		$banks_accounts->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function RenderRow() {
		...
		} elseif ($banks_accounts->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		$bInsertRow = $banks_accounts->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($banks_accounts->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$banks_accounts->id->setDbValue($conn->Insert_ID());
			$rsnew['id'] = $banks_accounts->id->DbValue;

			// Call Row Inserted event
			$banks_accounts->Row_Inserted($rsnew);
		}
		return $AddRow;
	}
