...
//
// Table class for consignees
//
class cconsignees {
	var $TableVar = 'consignees';
	...
	//
	function cconsignees() {
		global $Language;

		// id
		$this->id = new cField('consignees', 'consignees', 'x_id', 'id', '`id`', 3, -1, FALSE, '`id`', FALSE);
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] =& $this->id;

		// client_id
		$this->client_id = new cField('consignees', 'consignees', 'x_client_id', 'client_id', '`client_id`', 3, -1, FALSE, '`client_id`', FALSE);
		$this->client_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['client_id'] =& $this->client_id;

		// Customer_No
		$this->Customer_No = new cField('consignees', 'consignees', 'x_Customer_No', 'Customer_No', '`Customer_No`', 200, -1, FALSE, '`Customer_No`', FALSE);
		$this->fields['Customer_No'] =& $this->Customer_No;

		// Customer_Name
		$this->Customer_Name = new cField('consignees', 'consignees', 'x_Customer_Name', 'Customer_Name', '`Customer_Name`', 200, -1, FALSE, '`Customer_Name`', FALSE);
		$this->fields['Customer_Name'] =& $this->Customer_Name;

		// Address
		$this->Address = new cField('consignees', 'consignees', 'x_Address', 'Address', '`Address`', 200, -1, FALSE, '`Address`', FALSE);
		$this->fields['Address'] =& $this->Address;

		// Contact_Person
		$this->Contact_Person = new cField('consignees', 'consignees', 'x_Contact_Person', 'Contact_Person', '`Contact_Person`', 200, -1, FALSE, '`Contact_Person`', FALSE);
		$this->fields['Contact_Person'] =& $this->Contact_Person;

		// Contact_No
		$this->Contact_No = new cField('consignees', 'consignees', 'x_Contact_No', 'Contact_No', '`Contact_No`', 200, -1, FALSE, '`Contact_No`', FALSE);
		$this->fields['Contact_No'] =& $this->Contact_No;

		// Remarks
		$this->Remarks = new cField('consignees', 'consignees', 'x_Remarks', 'Remarks', '`Remarks`', 201, -1, FALSE, '`Remarks`', FALSE);
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `consignees` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		global $conn;
		$SQL = "UPDATE `consignees` SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `consignees` WHERE ";
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("consigneesview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "consigneesadd.php";
	...
	// Edit URL
	function EditUrl() {
		return $this->KeyUrl("consigneesedit.php", $this->UrlParm());
	}
	...
	// Copy URL
	function CopyUrl() {
		return $this->KeyUrl("consigneesadd.php", $this->UrlParm());
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("consigneesdelete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=consignees" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function LoadListRowValues(&$rs) {
		...
		$this->Customer_No->setDbValue($rs->fields('Customer_No'));
   ...
   // Common render codes
		...
		// Customer_No
		$this->Customer_No->CellCssStyle = ""; $this->Customer_No->CellCssClass = "";
		$this->Customer_No->CellAttrs = array(); $this->Customer_No->ViewAttrs = array(); $this->Customer_No->EditAttrs = array();

		// Customer_Name
		$this->Customer_Name->CellCssStyle = ""; $this->Customer_Name->CellCssClass = "";
		$this->Customer_Name->CellAttrs = array(); $this->Customer_Name->ViewAttrs = array(); $this->Customer_Name->EditAttrs = array();
		...
		// Customer_No
		$this->Customer_No->ViewValue = $this->Customer_No->CurrentValue;
		$this->Customer_No->CssStyle = "";
		$this->Customer_No->CssClass = "";
		$this->Customer_No->ViewCustomAttributes = "";

		// Customer_Name
		$this->Customer_Name->ViewValue = $this->Customer_Name->CurrentValue;
		$this->Customer_Name->CssStyle = "";
		$this->Customer_Name->CssClass = "";
		$this->Customer_Name->ViewCustomAttributes = "";

		// Address
		if (strval($this->Address->CurrentValue) <> "") {
			$sFilterWrk = "`Destination` = '" . ew_AdjustSql($this->Address->CurrentValue) . "'";
		...
		if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
			$rswrk = $conn->Execute($sSqlWrk);
			if ($rswrk && !$rswrk->EOF) { // Lookup values found
				$this->Address->ViewValue = $rswrk->fields('Destination');
				$rswrk->Close();
			} else {
				$this->Address->ViewValue = $this->Address->CurrentValue;
			}
		} else {
			$this->Address->ViewValue = NULL;
		}
		...
		// Customer_No
		$this->Customer_No->HrefValue = "";
		$this->Customer_No->TooltipValue = "";
