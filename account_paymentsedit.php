...
// Create page object
$account_payments_edit = new caccount_payments_edit();
$Page =& $account_payments_edit;

// Page init
$account_payments_edit->Page_Init();

// Page main
$account_payments_edit->Page_Main();
?>
...
// Create page object
var account_payments_edit = new ew_Page("account_payments_edit");

// page properties
account_payments_edit.PageID = "edit"; // page ID
account_payments_edit.FormID = "faccount_paymentsedit"; // form ID
var EW_PAGE_ID = account_payments_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
account_payments_edit.ValidateForm = function(fobj) {
...
account_payments_edit.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
account_payments_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
account_payments_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
account_payments_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $account_payments->TableCaption() ?><br><br>
<a href="<?php echo $account_payments->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$account_payments_edit->ShowMessage();
?>
<form name="faccount_paymentsedit" id="faccount_paymentsedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return account_payments_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="account_payments">
...
<?php if ($account_payments->id->Visible) { // id ?>
	<tr<?php echo $account_payments->id->RowAttributes ?>>
		...
		<td<?php echo $account_payments->id->CellAttributes() ?>><span id="el_id">
<div<?php echo $account_payments->id->ViewAttributes() ?>><?php echo $account_payments->id->EditValue ?></div><input type="hidden" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($account_payments->id->CurrentValue) ?>">
</span><?php echo $account_payments->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<?php $account_payments->Journal_Type_ID->EditAttrs["onchange"] = "ew_UpdateOpt('x_Journal_Account_ID','x_Journal_Type_ID',account_payments_edit.ar_x_Journal_Account_ID); " . @$account_payments->Journal_Type_ID->EditAttrs["onchange"]; ?>
...
<script type="text/javascript">
<!--
account_payments_edit.ar_x_Journal_Account_ID = [<?php echo $jswrk ?>];
...
<script language="JavaScript" type="text/javascript">
<!--
ew_UpdateOpts([['x_Journal_Account_ID','x_Journal_Type_ID',account_payments_edit.ar_x_Journal_Account_ID]]);
...
<?php
$account_payments_edit->Page_Terminate();
?>
<?php
...
class caccount_payments_edit {
	...
	// Page object name
	var $PageObjName = 'account_payments_edit';
	...
	//
	function caccount_payments_edit() {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$account_payments->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$account_payments->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$account_payments->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$account_payments->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($account_payments->id->CurrentValue == "")
			$this->Page_Terminate("account_paymentslist.php"); // Invalid key, return to list
		switch ($account_payments->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$account_payments->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$account_payments->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$this->RenderRow();
	}
	...
	function LoadFormValues() {
		...
		$account_payments->Modified->setFormValue($objForm->GetValue("x_Modified"));
		$account_payments->Modified->CurrentValue = ew_UnFormatDateTime($account_payments->Modified->CurrentValue, 6);
	...
	function RestoreFormValues() {
		...
		$account_payments->Modified->CurrentValue = $account_payments->Modified->FormValue;
		$account_payments->Modified->CurrentValue = ew_UnFormatDateTime($account_payments->Modified->CurrentValue, 6);
	...
	function RenderRow() {
		...
		} elseif ($account_payments->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// id
			$account_payments->id->EditCustomAttributes = "";
			$account_payments->id->EditValue = $account_payments->id->CurrentValue;
			$account_payments->id->CssStyle = "";
			$account_payments->id->CssClass = "";
			$account_payments->id->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// Modified
			$account_payments->Modified->SetDbValueDef($rsnew, ew_CurrentDateTime(), NULL);
			$rsnew['Modified'] =& $account_payments->Modified->DbValue;

			// total_invoice_items
			$account_payments->total_invoice_items->SetDbValueDef($rsnew, $account_payments->total_invoice_items->CurrentValue, 0, FALSE);

			// Call Row Updating event
			$bUpdateRow = $account_payments->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($account_payments->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$account_payments->Row_Updated($rsold, $rsnew);
		$rs->Close();
		return $EditRow;
	}
