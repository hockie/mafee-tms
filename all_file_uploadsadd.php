...
// Create page object
$all_file_uploads_add = new call_file_uploads_add();
$Page =& $all_file_uploads_add;

// Page init
$all_file_uploads_add->Page_Init();

// Page main
$all_file_uploads_add->Page_Main();
?>
...
// Create page object
var all_file_uploads_add = new ew_Page("all_file_uploads_add");

// page properties
all_file_uploads_add.PageID = "add"; // page ID
all_file_uploads_add.FormID = "fall_file_uploadsadd"; // form ID
var EW_PAGE_ID = all_file_uploads_add.PageID; // for backward compatibility

// extend page with ValidateForm function
all_file_uploads_add.ValidateForm = function(fobj) {
...
all_file_uploads_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
all_file_uploads_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
all_file_uploads_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
all_file_uploads_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $all_file_uploads->TableCaption() ?><br><br>
<a href="<?php echo $all_file_uploads->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$all_file_uploads_add->ShowMessage();
?>
<form name="fall_file_uploadsadd" id="fall_file_uploadsadd" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return all_file_uploads_add.ValidateForm(this);">
<p>
...
<?php
$all_file_uploads_add->Page_Terminate();
?>
<?php
...
class call_file_uploads_add {
	...
	// Page object name
	var $PageObjName = 'all_file_uploads_add';
	...
	//
	function call_file_uploads_add() {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $all_file_uploads->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->GetUploadFiles(); // Get upload files
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$all_file_uploads->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $all_file_uploads->CurrentAction = "C"; // Copy record
		  } else {
		    $all_file_uploads->CurrentAction = "I"; // Display blank record
		...
		switch ($all_file_uploads->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$all_file_uploads->SendEmail = TRUE; // Send email on add success
		...
		$all_file_uploads->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $all_file_uploads;
		$all_file_uploads->module->CurrentValue = $this->TableName;
		$all_file_uploads->File_Name->CurrentValue = NULL; // Clear file related field
		$all_file_uploads->Remarks->CurrentValue = "''";
		$all_file_uploads->Created->CurrentValue = "0000-00-00 00:00:00";
		$all_file_uploads->user_id->CurrentValue = 0;
		$all_file_uploads->file_id->CurrentValue = 0;
	}
	...
	function LoadFormValues() {
		...
		$all_file_uploads->Created->setFormValue($objForm->GetValue("x_Created"));
		$all_file_uploads->Created->CurrentValue = ew_UnFormatDateTime($all_file_uploads->Created->CurrentValue, 6);
	...
	function RestoreFormValues() {
		...
		$all_file_uploads->Created->CurrentValue = $all_file_uploads->Created->FormValue;
		$all_file_uploads->Created->CurrentValue = ew_UnFormatDateTime($all_file_uploads->Created->CurrentValue, 6);
	...
	function RenderRow() {
		...
		} elseif ($all_file_uploads->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		// module
		$all_file_uploads->module->SetDbValueDef($rsnew, $all_file_uploads->module->CurrentValue, "", TRUE);
		...
		// Remarks
		$all_file_uploads->Remarks->SetDbValueDef($rsnew, $all_file_uploads->Remarks->CurrentValue, "", TRUE);

		// Created
		$all_file_uploads->Created->SetDbValueDef($rsnew, ew_CurrentDateTime(), NULL);
		$rsnew['Created'] =& $all_file_uploads->Created->DbValue;
		...
		// file_id
		$all_file_uploads->file_id->SetDbValueDef($rsnew, $all_file_uploads->file_id->CurrentValue, 0, TRUE);

		// Call Row Inserting event
		$bInsertRow = $all_file_uploads->Row_Inserting($rsnew);
		if ($bInsertRow) {
			...
			$AddRow = $conn->Execute($all_file_uploads->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$all_file_uploads->id->setDbValue($conn->Insert_ID());
			$rsnew['id'] = $all_file_uploads->id->DbValue;

			// Call Row Inserted event
			$all_file_uploads->Row_Inserted($rsnew);
		}
