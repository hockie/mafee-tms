...
class cusers {
	...
	function cusers() {
		...
		// id
		$this->id = new cField('users', 'users', 'x_id', 'id', '`id`', 19, -1, FALSE, '`id`', FALSE);
		...
		// UserLevel
		$this->UserLevel = new cField('users', 'users', 'x_UserLevel', 'UserLevel', '`UserLevel`', 3, -1, FALSE, '`UserLevel`', FALSE);
		$this->UserLevel->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['UserLevel'] =& $this->UserLevel;

		// employee_ID
		$this->employee_ID = new cField('users', 'users', 'x_employee_ID', 'employee_ID', '`employee_ID`', 3, -1, FALSE, '`employee_ID`', FALSE);
   ...
   // Common render codes
		...
		// UserLevel
		$this->UserLevel->CellCssStyle = ""; $this->UserLevel->CellCssClass = "";
		$this->UserLevel->CellAttrs = array(); $this->UserLevel->ViewAttrs = array(); $this->UserLevel->EditAttrs = array();
		...
		if (strval($this->UserLevel->CurrentValue) <> "") {
			$sFilterWrk = "`userlevelid` = " . ew_AdjustSql($this->UserLevel->CurrentValue) . "";
		...
		if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
			...
			if ($rswrk && !$rswrk->EOF) { // Lookup values found
				$this->UserLevel->ViewValue = $rswrk->fields('userlevelname');
				$rswrk->Close();
			} else {
		...
		$this->UserLevel->CssStyle = "";
		$this->UserLevel->CssClass = "";
