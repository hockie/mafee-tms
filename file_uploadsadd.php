...
// Create page object
$file_uploads_add = new cfile_uploads_add();
$Page =& $file_uploads_add;

// Page init
$file_uploads_add->Page_Init();

// Page main
$file_uploads_add->Page_Main();
?>
...
// Create page object
var file_uploads_add = new ew_Page("file_uploads_add");

// page properties
file_uploads_add.PageID = "add"; // page ID
file_uploads_add.FormID = "ffile_uploadsadd"; // form ID
var EW_PAGE_ID = file_uploads_add.PageID; // for backward compatibility

// extend page with ValidateForm function
file_uploads_add.ValidateForm = function(fobj) {
...
file_uploads_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
file_uploads_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
file_uploads_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
file_uploads_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $file_uploads->TableCaption() ?><br><br>
<a href="<?php echo $file_uploads->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$file_uploads_add->ShowMessage();
?>
<form name="ffile_uploadsadd" id="ffile_uploadsadd" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return file_uploads_add.ValidateForm(this);">
<p>
...
<?php
$file_uploads_add->Page_Terminate();
?>
<?php
...
class cfile_uploads_add {
	...
	// Page object name
	var $PageObjName = 'file_uploads_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $file_uploads->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->GetUploadFiles(); // Get upload files
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$file_uploads->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $file_uploads->CurrentAction = "C"; // Copy record
		  } else {
		    $file_uploads->CurrentAction = "I"; // Display blank record
		...
		switch ($file_uploads->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$file_uploads->SendEmail = TRUE; // Send email on add success
		...
		$file_uploads->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $file_uploads;
		$file_uploads->Filename->CurrentValue = NULL; // Clear file related field
	}
	...
	function RenderRow() {
		...
		} elseif ($file_uploads->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		$bInsertRow = $file_uploads->Row_Inserting($rsnew);
		if ($bInsertRow) {
			...
			$AddRow = $conn->Execute($file_uploads->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$file_uploads->id->setDbValue($conn->Insert_ID());
			$rsnew['id'] = $file_uploads->id->DbValue;

			// Call Row Inserted event
			$file_uploads->Row_Inserted($rsnew);
		}
