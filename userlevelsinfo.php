...
class cuserlevels {
	...
	//
	function cuserlevels() {
		global $Language;

		// userlevelid
		$this->userlevelid = new cField('userlevels', 'userlevels', 'x_userlevelid', 'userlevelid', '`userlevelid`', 3, -1, FALSE, '`userlevelid`', FALSE);
		$this->userlevelid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['userlevelid'] =& $this->userlevelid;

		// userlevelname
		$this->userlevelname = new cField('userlevels', 'userlevels', 'x_userlevelname', 'userlevelname', '`userlevelname`', 200, -1, FALSE, '`userlevelname`', FALSE);
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `userlevels` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		global $conn;
		$SQL = "UPDATE `userlevels` SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `userlevels` WHERE ";
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("userlevelsview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "userlevelsadd.php";
	...
	// Edit URL
	function EditUrl() {
		return $this->KeyUrl("userlevelsedit.php", $this->UrlParm());
	}
	...
	// Copy URL
	function CopyUrl() {
		return $this->KeyUrl("userlevelsadd.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=userlevels" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
   ...
   // Common render codes
		...
		// userlevelname
		$this->userlevelname->CellCssStyle = ""; $this->userlevelname->CellCssClass = "";
		$this->userlevelname->CellAttrs = array(); $this->userlevelname->ViewAttrs = array(); $this->userlevelname->EditAttrs = array();
		...
		$this->userlevelname->CssStyle = "";
		$this->userlevelname->CssClass = "";
