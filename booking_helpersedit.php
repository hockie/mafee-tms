...
// Create page object
$booking_helpers_edit = new cbooking_helpers_edit();
$Page =& $booking_helpers_edit;

// Page init
$booking_helpers_edit->Page_Init();

// Page main
$booking_helpers_edit->Page_Main();
?>
...
// Create page object
var booking_helpers_edit = new ew_Page("booking_helpers_edit");

// page properties
booking_helpers_edit.PageID = "edit"; // page ID
booking_helpers_edit.FormID = "fbooking_helpersedit"; // form ID
var EW_PAGE_ID = booking_helpers_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
booking_helpers_edit.ValidateForm = function(fobj) {
...
booking_helpers_edit.Form_CustomValidate =  
...
booking_helpers_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
booking_helpers_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $booking_helpers->TableCaption() ?><br><br>
<a href="<?php echo $booking_helpers->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$booking_helpers_edit->ShowMessage();
?>
<form name="fbooking_helpersedit" id="fbooking_helpersedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return booking_helpers_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="booking_helpers">
...
<?php if ($booking_helpers->id->Visible) { // id ?>
	<tr<?php echo $booking_helpers->id->RowAttributes ?>>
		...
		<td<?php echo $booking_helpers->id->CellAttributes() ?>><span id="el_id">
<div<?php echo $booking_helpers->id->ViewAttributes() ?>><?php echo $booking_helpers->id->EditValue ?></div><input type="hidden" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($booking_helpers->id->CurrentValue) ?>">
</span><?php echo $booking_helpers->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<?php
$booking_helpers_edit->Page_Terminate();
?>
<?php
...
class cbooking_helpers_edit {
	...
	// Page object name
	var $PageObjName = 'booking_helpers_edit';
	...
	//
	function cbooking_helpers_edit() {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$booking_helpers->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$booking_helpers->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$booking_helpers->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$booking_helpers->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($booking_helpers->id->CurrentValue == "")
			$this->Page_Terminate("booking_helperslist.php"); // Invalid key, return to list
		switch ($booking_helpers->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$booking_helpers->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$booking_helpers->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		} elseif ($booking_helpers->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// id
			$booking_helpers->id->EditCustomAttributes = "";
			$booking_helpers->id->EditValue = $booking_helpers->id->CurrentValue;
			$booking_helpers->id->CssStyle = "";
			$booking_helpers->id->CssClass = "";
			$booking_helpers->id->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$bUpdateRow = $booking_helpers->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($booking_helpers->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$booking_helpers->Row_Updated($rsold, $rsnew);
		$rs->Close();
		return $EditRow;
	}
