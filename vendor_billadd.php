...
// Create page object
$vendor_bill_add = new cvendor_bill_add();
$Page =& $vendor_bill_add;

// Page init
$vendor_bill_add->Page_Init();

// Page main
$vendor_bill_add->Page_Main();
?>
...
// Create page object
var vendor_bill_add = new ew_Page("vendor_bill_add");

// page properties
vendor_bill_add.PageID = "add"; // page ID
vendor_bill_add.FormID = "fvendor_billadd"; // form ID
var EW_PAGE_ID = vendor_bill_add.PageID; // for backward compatibility

// extend page with ValidateForm function
vendor_bill_add.ValidateForm = function(fobj) {
...
vendor_bill_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
vendor_bill_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
vendor_bill_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
vendor_bill_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $vendor_bill->TableCaption() ?><br><br>
<a href="<?php echo $vendor_bill->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$vendor_bill_add->ShowMessage();
?>
<form name="fvendor_billadd" id="fvendor_billadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return vendor_bill_add.ValidateForm(this);">
<p>
...
<?php
$vendor_bill_add->Page_Terminate();
?>
<?php
...
class cvendor_bill_add {
	...
	// Page object name
	var $PageObjName = 'vendor_bill_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $vendor_bill->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$vendor_bill->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $vendor_bill->CurrentAction = "C"; // Copy record
		  } else {
		    $vendor_bill->CurrentAction = "I"; // Display blank record
		...
		switch ($vendor_bill->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$vendor_bill->SendEmail = TRUE; // Send email on add success
		...
		$vendor_bill->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadFormValues() {
		...
		$vendor_bill->created->setFormValue($objForm->GetValue("x_created"));
		$vendor_bill->created->CurrentValue = ew_UnFormatDateTime($vendor_bill->created->CurrentValue, 6);
	...
	function RestoreFormValues() {
		...
		$vendor_bill->created->CurrentValue = $vendor_bill->created->FormValue;
		$vendor_bill->created->CurrentValue = ew_UnFormatDateTime($vendor_bill->created->CurrentValue, 6);
	}
	...
	function RenderRow() {
		...
		} elseif ($vendor_bill->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		// created
		$vendor_bill->created->SetDbValueDef($rsnew, ew_CurrentDateTime(), NULL);
		$rsnew['created'] =& $vendor_bill->created->DbValue;

		// Call Row Inserting event
		$bInsertRow = $vendor_bill->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($vendor_bill->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$vendor_bill->id->setDbValue($conn->Insert_ID());
			$rsnew['id'] = $vendor_bill->id->DbValue;

			// Call Row Inserted event
			$vendor_bill->Row_Inserted($rsnew);
		}
		return $AddRow;
	}
