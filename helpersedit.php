...
// Create page object
$helpers_edit = new chelpers_edit();
$Page =& $helpers_edit;

// Page init
$helpers_edit->Page_Init();

// Page main
$helpers_edit->Page_Main();
?>
...
// Create page object
var helpers_edit = new ew_Page("helpers_edit");

// page properties
helpers_edit.PageID = "edit"; // page ID
helpers_edit.FormID = "fhelpersedit"; // form ID
var EW_PAGE_ID = helpers_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
helpers_edit.ValidateForm = function(fobj) {
...
helpers_edit.Form_CustomValidate =  
...
helpers_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
helpers_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $helpers->TableCaption() ?><br><br>
<a href="<?php echo $helpers->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$helpers_edit->ShowMessage();
?>
<form name="fhelpersedit" id="fhelpersedit" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return helpers_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="helpers">
...
<?php if ($helpers->id->Visible) { // id ?>
	<tr<?php echo $helpers->id->RowAttributes ?>>
		...
		<td<?php echo $helpers->id->CellAttributes() ?>><span id="el_id">
<div<?php echo $helpers->id->ViewAttributes() ?>><?php echo $helpers->id->EditValue ?></div><input type="hidden" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($helpers->id->CurrentValue) ?>">
</span><?php echo $helpers->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<div id="old_x_Uploads">
<?php if ($helpers->Uploads->HrefValue <> "" || $helpers->Uploads->TooltipValue <> "") { ?>
<?php if (!empty($helpers->Uploads->Upload->DbValue)) { ?>
<a href="<?php echo $helpers->Uploads->HrefValue ?>"><?php echo $helpers->Uploads->EditValue ?></a>
<?php } elseif (!in_array($helpers->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } else { ?>
<?php if (!empty($helpers->Uploads->Upload->DbValue)) { ?>
<?php echo $helpers->Uploads->EditValue ?>
<?php } elseif (!in_array($helpers->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } ?>
</div>
<div id="new_x_Uploads">
<?php if (!empty($helpers->Uploads->Upload->DbValue)) { ?>
<label><input type="radio" name="a_Uploads" id="a_Uploads" value="1" checked="checked"><?php echo $Language->Phrase("Keep") ?></label>&nbsp;
<label><input type="radio" name="a_Uploads" id="a_Uploads" value="2"><?php echo $Language->Phrase("Remove") ?></label>&nbsp;
<label><input type="radio" name="a_Uploads" id="a_Uploads" value="3"><?php echo $Language->Phrase("Replace") ?><br></label>
<?php $helpers->Uploads->EditAttrs["onchange"] = "this.form.a_Uploads[2].checked=true;" . @$helpers->Uploads->EditAttrs["onchange"]; ?>
<?php } else { ?>
<input type="hidden" name="a_Uploads" id="a_Uploads" value="3">
<?php } ?>
...
class chelpers_edit {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$helpers->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->GetUploadFiles(); // Get upload files
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$helpers->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$helpers->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$helpers->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($helpers->id->CurrentValue == "")
			$this->Page_Terminate("helperslist.php"); // Invalid key, return to list
		switch ($helpers->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$helpers->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$helpers->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		if ($helpers->RowType == EW_ROWTYPE_VIEW) { // View row
		...
		} elseif ($helpers->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// id
			$helpers->id->EditCustomAttributes = "";
			$helpers->id->EditValue = $helpers->id->CurrentValue;
			$helpers->id->CssStyle = "";
			$helpers->id->CssClass = "";
			$helpers->id->ViewCustomAttributes = "";
			...
			// Uploads
			if (!ew_Empty($helpers->Uploads->Upload->DbValue)) {
				$helpers->Uploads->HrefValue = ew_UploadPathEx(FALSE, $helpers->Uploads->UploadPath) . ((!empty($helpers->Uploads->EditValue)) ? $helpers->Uploads->EditValue : $helpers->Uploads->CurrentValue);
				if ($helpers->Export <> "") $helpers->Uploads->HrefValue = ew_ConvertFullUrl($helpers->Uploads->HrefValue);
			} else {
				$helpers->Uploads->HrefValue = "";
			}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$helpers->Uploads->Upload->SaveToSession(); // Save file value to Session
						if ($helpers->Uploads->Upload->Action == "2" || $helpers->Uploads->Upload->Action == "3") { // Update/Remove
			$helpers->Uploads->Upload->DbValue = $rs->fields('Uploads'); // Get original value
		...
		if ($EditRow)
			$helpers->Row_Updated($rsold, $rsnew);
		$rs->Close();
