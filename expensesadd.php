...
// Create page object
$expenses_add = new cexpenses_add();
$Page =& $expenses_add;

// Page init
$expenses_add->Page_Init();

// Page main
$expenses_add->Page_Main();
?>
...
// Create page object
var expenses_add = new ew_Page("expenses_add");

// page properties
expenses_add.PageID = "add"; // page ID
expenses_add.FormID = "fexpensesadd"; // form ID
var EW_PAGE_ID = expenses_add.PageID; // for backward compatibility

// extend page with ValidateForm function
expenses_add.ValidateForm = function(fobj) {
...
expenses_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
expenses_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
expenses_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
expenses_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $expenses->TableCaption() ?><br><br>
<a href="<?php echo $expenses->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$expenses_add->ShowMessage();
?>
<form name="fexpensesadd" id="fexpensesadd" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return expenses_add.ValidateForm(this);">
<p>
...
<?php $expenses->approver->EditAttrs["onchange"] = "ew_UpdateOpt('x_employee_id','x_approver',expenses_add.ar_x_employee_id); " . @$expenses->approver->EditAttrs["onchange"]; ?>
...
<script type="text/javascript">
<!--
expenses_add.ar_x_employee_id = [<?php echo $jswrk ?>];
...
<script language="JavaScript" type="text/javascript">
<!--
ew_UpdateOpts([['x_employee_id','x_approver',expenses_add.ar_x_employee_id]]);
...
class cexpenses_add {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $expenses->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->GetUploadFiles(); // Get upload files
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$expenses->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $expenses->CurrentAction = "C"; // Copy record
		  } else {
		    $expenses->CurrentAction = "I"; // Display blank record
		...
		switch ($expenses->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$expenses->SendEmail = TRUE; // Send email on add success
		...
		$expenses->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $expenses;
		$expenses->Amount->CurrentValue = 0.00;
		$expenses->File_Upload->CurrentValue = NULL; // Clear file related field
	}
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm, $expenses;
		$expenses->Date_Created->setFormValue($objForm->GetValue("x_Date_Created"));
		$expenses->Date_Created->CurrentValue = ew_UnFormatDateTime($expenses->Date_Created->CurrentValue, 6);
	...
	function RestoreFormValues() {
		...
		$expenses->Date_Created->CurrentValue = $expenses->Date_Created->FormValue;
		$expenses->Date_Created->CurrentValue = ew_UnFormatDateTime($expenses->Date_Created->CurrentValue, 6);
	...
	function RenderRow() {
		...
		} elseif ($expenses->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		// Date_Created
		$expenses->Date_Created->SetDbValueDef($rsnew, ew_CurrentDate(), NULL);
		$rsnew['Date_Created'] =& $expenses->Date_Created->DbValue;
		...
		$bInsertRow = $expenses->Row_Inserting($rsnew);
		if ($bInsertRow) {
			...
			$AddRow = $conn->Execute($expenses->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$expenses->id->setDbValue($conn->Insert_ID());
			$rsnew['id'] = $expenses->id->DbValue;

			// Call Row Inserted event
			$expenses->Row_Inserted($rsnew);
		}
