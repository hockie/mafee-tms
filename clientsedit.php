...
clients_edit.FormID = "fclientsedit"; // form ID
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $clients->TableCaption() ?><br><br>
...
<a href="<?php echo $clients->File_Upload->HrefValue ?>"><?php echo $clients->File_Upload->EditValue ?></a>
<?php } elseif (!in_array($clients->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } else { ?>
<?php if (!empty($clients->File_Upload->Upload->DbValue)) { ?>
<?php echo $clients->File_Upload->EditValue ?>
<?php } elseif (!in_array($clients->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } ?>
...
<?php $clients->File_Upload->EditAttrs["onchange"] = "this.form.a_File_Upload[2].checked=true;" . @$clients->File_Upload->EditAttrs["onchange"]; ?>
<?php } else { ?>
...
class cclients_edit {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			...
			if (!$this->ValidateForm()) {
				...
				$clients->EventCancelled = TRUE; // Event cancelled
	...
	function RenderRow() {
		...
		if ($clients->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			// File_Upload
			if (!ew_Empty($clients->File_Upload->Upload->DbValue)) {
				$clients->File_Upload->HrefValue = ew_UploadPathEx(FALSE, $clients->File_Upload->UploadPath) . ((!empty($clients->File_Upload->EditValue)) ? $clients->File_Upload->EditValue : $clients->File_Upload->CurrentValue);
				if ($clients->Export <> "") $clients->File_Upload->HrefValue = ew_ConvertFullUrl($clients->File_Upload->HrefValue);
			} else {
				$clients->File_Upload->HrefValue = "";
			}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$clients->File_Upload->Upload->SaveToSession(); // Save file value to Session
						if ($clients->File_Upload->Upload->Action == "2" || $clients->File_Upload->Upload->Action == "3") { // Update/Remove
			$clients->File_Upload->Upload->DbValue = $rs->fields('File_Upload'); // Get original value
